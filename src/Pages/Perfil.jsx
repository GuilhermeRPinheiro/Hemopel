...
function Perfil() {
  ...
  // Busca campanhas da API quando painel for aberto
  useEffect(() => {
    if (showCampanhas) {
      loadCampanhas()
    }
  ...
  async function handleAddCampanha() {
    ...
    if (nome) {
      const novaCampanha = { nome, progresso: 0 }
  ...
  // Separa as campanhas em ativas (<100) e concluídas (100)
    ...
    <section className="bg-white min-h-screen py-6 px-4 md:px-[5rem] flex flex-col items-center md:items-start">
      <div className="flex flex-col md:flex-row gap-6 w-full">
        ...
        <div className="bg-[#91302A] w-full h-[20rem] md:h-[43rem] rounded-3xl flex flex-col items-center relative">
          {/* Painel de campanhas CRUD (só aparece se showCampanhas for true) */}
          {showCampanhas && (
            <div className="absolute top-0 left-0 w-full h-full bg-white bg-opacity-90 flex flex-col p-6 rounded-3xl z-10 overflow-auto">
                      ...
                      <div key={idx} className="bg-[#FFD8CC] rounded-lg shadow p-4 mb-4 flex flex-col">
                        ...
                        {isAdmin() && (
                          <div className="flex gap-2 mt-2">
                            <button
                              className="bg-[#4FD31F] px-4 py-1 rounded font-bold text-white"
                              onClick={() => handleEditProgresso(campanha)}
                            >
                              Editar Progresso
                            </button>
                            <button
                              className="bg-[#FFA500] px-4 py-1 rounded font-bold text-white"
                              onClick={() => handleConcluir(campanha)}
                            >
                              Concluir
                            </button>
                            <button
                              className="bg-[#C8392F] px-4 py-1 rounded font-bold text-white"
              ...
              <button
                className="bg-[#91302A] text-white rounded-xl px-6 py-2 font-bold mt-4 w-full"
                onClick={() => setShowCampanhas(false)}
              >
                Fechar
              </button>
            </div>
